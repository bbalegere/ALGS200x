...
public class PlacingParentheses {
    ...
    private static int[] getMinAndMax(String exp, int i, int j, int[][] min, int[][] max) {
        ...
        for (int index = i; index <= j - 1; index++) {
            ...
            long b = eval(min[i][index], max[index + 1][j], oper);
            long c = eval(max[i][index], min[index + 1][j], oper);
            long d = eval(max[i][index], max[index + 1][j], oper);
            res[0] = (int) Math.min(a, Math.min(b, Math.min(c, Math.min(d, res[0]))));
            res[1] = (int) Math.max(a, Math.max(b, Math.max(c, Math.max(d, res[1]))));
        }
        return res;
    }
